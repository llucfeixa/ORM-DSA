...
public class QueryHelperTest {
    ...
    public void testCreateQueryINSERT() throws NoSuchFieldException, InvocationTargetException, IllegalAccessException {
        User user = new User("1", "Lluc", "Feixa", "29/12/2001", "llucfeixa@gmail.com", "123");
        String query = QueryHelper.createQueryINSERT(user);
        Assert.assertEquals("INSERT INTO User (userId, userName, userSurname, userBirth, email, password) VALUES (?, ?, ?, ?, ?, ?)", query);
    }
    ...
    public void testCreateQueryUPDATE() {
        ...
        Assert.assertEquals("UPDATE User SET userId = ?, userName = ?, userSurname = ?, userBirth = ?, email = ?, password = ? WHERE userId = ?", query);
    }
